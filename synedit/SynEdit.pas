...
$Id: SynEdit.pas,v 1.386 2004/08/22 20:04:53 markonjezic Exp $
...
const
  SYNEDIT_DEFAULT_OPTIONS = [eoAutoIndent, eoDragDropEditing, eoEnhanceEndKey,
    ...
    eoTrimTrailingSpaces, eoSmartTabDelete, eoGroupUndo];
...
// use scAll to update a statusbar when another TCustomSynEdit got the focus
  ...
  protected
    ...
    procedure LinesDeleted(FirstLine, Count: integer;
      AddUndo: Boolean); virtual; abstract;
  protected
...
{$ENDIF}
    ...
    procedure DoLinesDeleted(FirstLine, Count: integer; AddUndo: Boolean);
    ...
    procedure InsertBlock(const BB, BE: TBufferCoord; ChangeStr: PChar;
...
{$ENDIF}
    ...
    procedure DrawIndentGuides(aLine, aRow, nX: Integer; rcLine: TRect);
    procedure PaintIndentGuides;
  ...
  if not SelAvail then
    ...
    case fActiveSelectionMode of
      smNormal:
        if (First = Last) then
        begin
          US := GetUncollapsedStrings;
          Inc(TotalLen, ColTo - ColFrom);
          if (Length(US[GetRealLineNumber(First + 1) - 1]) < ColTo)
            and (FoldRangeForLine(First) <> nil)
            and (FoldRangeForLine(First).Collapsed) then
          begin
            Cnt := 1;
            for i := GetRealLineNumber(First + 1) to
              GetRealLineNumber(Last + 2) - 2 do
            begin
              Inc(TotalLen, Length(TrimRight(US[i])));
              Inc(Cnt);
            end;
            Inc(TotalLen, Cnt * Length(sLineBreak));
          end;
          SetLength(Result, TotalLen);
          P := PChar(Result);
          CopyAndForward(TrimRight(US[GetRealLineNumber(First + 1) - 1]),
          ...
          begin
            ...
            for i := GetRealLineNumber(First + 1) to
              GetRealLineNumber(Last + 2) - 2 do
            begin
              CopyAndForward(TrimRight(US[i]), 1, MaxInt, P);
              CopyAndForward(sLineBreak, 1, MaxInt, P);
            end;
          end;
          US.Free;
          ...
          Cnt := 0;
          Inc(TotalLen,
            Length(TrimRight(US[GetRealLineNumber(First + 1) - 1])));
          Inc(Cnt);
          for i := GetRealLineNumber(First + 1) to
            GetRealLineNumber(Last + 1) - 2 do
          begin
            Inc(TotalLen, Length(TrimRight(US[i])));
            Inc(Cnt);
          end;
          ...
          CopyAndForward(TrimRight(US[GetRealLineNumber(Last + 1) - 1]), 1,
            ColTo - 1, P);
          US.Free;
        end;
      smColumn:
      ...
      smLine:
        begin
          ...
          // step1: calculate total length of result string
          US := GetUncollapsedStrings;
          for i := GetRealLineNumber(First + 1) - 1 to
            GetRealLineNumber(Last + 2) - 2 do
            Inc(TotalLen, Length(TrimRight(US[i])) + Length(sLineBreak));
...
begin
  //### Code Folding ###
	if not CanExecuteInLine(CaretY) then
  	Exit;
  //### End Code Folding ###
  ...
  begin
    ...
    begin
      ...
      for i := 0 to fAllFoldRanges.AllCount - 1 do
        if fAllFoldRanges[i].Collapsed then
          Inc(iLines, fAllFoldRanges[i].LinesCollapsed + 1);
      fGutter.AutoSizeDigitCount(iLines);
      //### End Code Folding ###
    end;
  ...
  begin
  	...
  	if TSynCompletionProposal(fHint).Tag = 1 then
		begin
			...
			TSynCompletionProposal(fHint).InsertList.Clear;
		end;
  end;
  //### End Code Folding ###
begin
  ...
  begin
    ...
    begin
      ...
      begin
          ...
          Pt.X := ScrolledXBy + (Mouse.CursorPos.X - X + Pt.X + 4);
          Pt.Y := Mouse.CursorPos.Y - Y + Pt.Y + 2;

        	if TSynCompletionProposal(fHint).Tag = 0 then
        	begin
        		TSynCompletionProposal(fHint).ItemList.Clear;
            S := StringReplace(Lines[RowToLine(P.Row) - 1], #9,
              StringOfChar(#32, TabWidth), [rfReplaceAll]);
            if S <> '' then
              Delete(S, StrLen(PChar(S)) - 5, StrLen(PChar(S)));
            TSynCompletionProposal(fHint).ItemList.Add(S);

            Cnt := FoldRange.CollapsedLines.Count - 1;
            if Cnt > 40 then
            begin
              for i := Cnt downto Cnt - 40 do
                TSynCompletionProposal(fHint).ItemList.Add(
                  StringReplace(FoldRange.CollapsedLines[i], #9,
                  StringOfChar(#32, TabWidth), [rfReplaceAll]) );
            ...
            TSynCompletionProposal(fHint).Font.Name := 'courier new';
            TSynCompletionProposal(fHint).Font.Size := 9;
      ...
      begin
        ...
        // http://mystix.projects.waterproof.fr/IssueView.php?Id=10
      	if Gutter.ShowLineNumbers then
					xpos := Gutter.LeftOffset + Max(GetAutoSizeDigitCount, Gutter.DigitCount) * Gutter.CharWidth + 4
        else
...
{$ENDIF}
    try
      ...
      // prepare the cfRect initially
      rcLine := AClip;

      //### Code Folding ###
      if CodeFolding.Enabled then
      	rcLine.Right := Gutter.LeftOffset
        	+ Max(GetAutoSizeDigitCount, Gutter.DigitCount) * Gutter.CharWidth + 4
      else
      ...
      begin
        ...
        if CodeFolding.Enabled then
					s := IntToStr(GetRealLineNumber(cLine));
        //### End Code Folding ###
...
{$ENDIF}
  ...
  begin
		cfRect := AClip;
		cfRect.Left := xpos;
		cfRect.Right := xpos + Gutter.RightOffset - 4;

		// Paint fold bar
		Canvas.Brush.Color := CodeFolding.FolderBarColor;
		Canvas.FillRect(cfRect);
		Canvas.Brush.Color := fGutter.Color;

    // Paint folding bar dividing line
    Canvas.Pen.Color := clBtnShadow;
    Canvas.MoveTo(cfRect.Left, cfRect.Top);
    Canvas.LineTo(cfRect.Left, cfRect.Bottom);
	end;
  //### End Code Folding ###
...
{$ENDIF}
  ...
  var
    ...
    begin
      ...
      begin
        ...
        while (pTok^ = #32) do
          Inc(pTok);
        bIsSpaces := (pTok^ = #0) and
          (pTok - PChar(Token) = Length(Token));
      end;
      Result := bIsSpaces;
    end;
  ...
  begin
    ...
    for nLine := vFirstLine to vLastLine do begin
    	//### Code Folding ###
      if CodeFolding.Enabled then
      ...
      for cRow := vStartRow to vEndRow do begin
        ...
        if bAnySelection and (cRow >= vSelStart.Row) and (cRow <= vSelEnd.Row) then begin
          ...
          if (ActiveSelectionMode = smColumn) or
            ((ActiveSelectionMode = smNormal) and (cRow = vSelStart.Row))
          then
          ...
          if (ActiveSelectionMode = smColumn) or
            ((ActiveSelectionMode = smNormal) and (cRow = vSelEnd.Row))
          then
...
{$ENDIF}
          ...
          begin
        		...
        		OldColor := Canvas.Brush.Color;
      			Canvas.Brush.Color := clSilver;
						...
						if Gutter.ShowLineNumbers then
							FoldRange.HintMarkLeft := ((nTokenPos + (-4)) * CharWidth)
								+ Gutter.LeftOffset + (GetAutoSizeDigitCount * Gutter.CharWidth)
								+ Gutter.RightOffset
						else
            	FoldRange.HintMarkLeft := ((nTokenPos + (-4)) * CharWidth)
            ...
            begin
            	OffsetRect(rcCollapseMark, -ScrolledXBy, 0);
        			...
        			Y := rcCollapseMark.Top + ( ( (rcCollapseMark.Bottom - rcCollapseMark.Top) div 5 ) * 3 );
        ...
        DrawIndentGuides(nLine, cRow, ScrolledXBy, rcLine);
...
procedure TCustomSynEdit.DrawIndentGuides(aLine, aRow, nX: Integer;
  rcLine: TRect);
var
  OldColor, PenColor: TColor;
  X, Y: Integer;
...
begin
  ...
  if (CodeFolding.Enabled) and (CodeFolding.IndentGuides)
  and ((aRow = 1) or (RowToLine(aRow) <> RowToLine(aRow - 1))) then
  begin
      for i := fAllFoldRanges.AllCount - 1 downto 0 do
        if (fAllFoldRanges[i].Level > HighlightLevel)
        and (fAllFoldRanges[i].FromLine <= CaretY)
        and (fAllFoldRanges[i].ToLine >= CaretY) then
          HighlightLevel := fAllFoldRanges[i].Level;

      for i := 0 to fAllFoldRanges.AllCount - 1 do
      begin
        FoldRange := fAllFoldRanges[i];

        if (not FoldRange.Collapsed) and (not FoldRange.ParentCollapsed)
        and (FoldRange.FromLine < aLine) and (FoldRange.ToLine > aLine) then
        begin
          Y := rcLine.Top;
          X := GetLineIndentChars(Lines, FoldRange.FromLine - 1) * fCharWidth;

          if (FoldRange.Level = HighlightLevel)
          and (FoldRange.FromLine <= CaretY) and (FoldRange.ToLine >= CaretY) then
            Canvas.Pen.Color := clBlack
          else
          ...
          if X - nX > 0 then
          begin
            X := fGutter.RealGutterWidth(fGutter.CharWidth) + X - nX;
...
procedure TCustomSynEdit.PaintIndentGuides;
var
  rcLine: TRect;
  i, j, nX: Integer;
begin
  try
    HideCaret;
    rcLine.Left := fGutter.RealGutterWidth(fGutter.CharWidth);
    rcLine.Right := fCharsInWindow * fCharWidth + rcLine.Left;
    rcLine.Top := 0;
    rcLine.Bottom := fTextHeight;
    nX := (fLeftChar - 1) * fCharWidth;

    for i := RowToLine(fTopLine) to RowToLine(fTopLine) + fLinesInWindow do
      for j := LineToRow(i) to LineToRow(i + 1) - 1 do
      begin
        DrawIndentGuides(i, j, nX, rcLine);
        rcLine.Top := rcLine.Bottom;
        Inc(rcLine.Bottom, fTextHeight);
      end;
  finally
...
{$ENDIF}
      ...
      fBlockBegin := vStartOfBlock;
      fBlockEnd := vEndOfBlock;
      SetSelTextPrimitive(GetClipboardText(Clipboard));
...
{$ELSE}
  ...
  begin
    ...
    for cRun := Value.Char to Length(TempString) -1 do
    ...
    { search BlockBegin }
    vBlockBegin.Char := 1;
    for cRun := Value.Char -1 downto 0 do
      if not(TempString[cRun] in IdChars) then
      begin
          ...
          case Item.ChangeSelMode of
            smNormal:
              if Item.ChangeStartPos.Line = Item.ChangeEndPos.Line then
                TempString := Copy(Lines[Item.ChangeStartPos.Line - 1],
                  Item.ChangeStartPos.Char,
                  Item.ChangeEndPos.Char - Item.ChangeStartPos.Char)
              else
              begin
                TempString := Copy(Lines[Item.ChangeStartPos.Line - 1],
                  Item.ChangeStartPos.Char, MaxInt) + #13#10;
                for i := Item.ChangeStartPos.Line + 1 to
                  Item.ChangeEndPos.Line - 1 do
                  TempString := TempString + Lines[i - 1] + #13#10;
                TempString := TempString +
                  Copy(Lines[Item.ChangeEndPos.Line - 1], 1,
                  Item.ChangeEndPos.Char - 1);
              end;
            smColumn:
              for i := Item.ChangeStartPos.Line to Item.ChangeEndPos.Line do
                TempString := TempString + Copy(Lines[i - 1],
                  Item.ChangeStartPos.Char,
                  Item.ChangeEndPos.Char - Item.ChangeStartPos.Char) + #13#10;
            smLine:
              for i := Item.ChangeStartPos.Line to Item.ChangeEndPos.Line do
                TempString := TempString + Lines[i - 1] + #13#10;
          end;
...
begin
  ...
  try
    ...
    case Item.ChangeReason of
      ...
      crDeleteCollapsedFold:
      	begin
          if fAllFoldRanges.AllCount > 0 then
          begin
            for i := 0 to fAllFoldRanges.AllCount - 1 do
              if fAllFoldRanges[i].FromLine >=
                TSynEditFoldRange(Item.ChangeData).FromLine then
              begin
              ...
              else if i = fAllFoldRanges.AllCount - 1 then
              begin
                fAllFoldRanges.AddFold(Item.ChangeData);
                fRedoList.AddChange(Item.ChangeReason, Item.ChangeStartPos,
                  Item.ChangeEndPos, '', Item.ChangeSelMode,
                  fAllFoldRanges[fAllFoldRanges.AllCount - 1],
                  fAllFoldRanges.AllCount - 1);
              end;
          end
          else
          begin
...
begin
  ...
  begin
    ...
    if (IsKeywordAtCursorPos)
    and ((Command = ecChar) or (Command = ecDeleteLastChar)
    or (Command = ecDeleteChar) or (Command = ecLineBreak)) then
    	fNeedToRescan := True;
    //### End Code Folding ###
    ...
    case fActiveSelectionMode of
      ...
      smColumn:
        for i := BB.Line to BE.Line do
          vSelText := vSelText + Copy(Lines[i - 1], BB.Char,
            BE.Char - BB.Char) + #13#10;
      smLine:
        for i := BB.Line to BE.Line do
          vSelText := vSelText + Lines[i - 1] + #13#10;
    end;
  ...
  try
    ...
    begin
      ...
      case fActiveSelectionMode of
        smNormal:
          if BB.Line = BE.Line then
            TempString := Copy(Lines[BB.Line - 1], BB.Char, BE.Char - BB.Char)
          else
          begin
            TempString := Copy(Lines[BB.Line - 1], BB.Char, MaxInt) + #13#10;
            for x := BB.Line + 1 to BE.Line - 1 do
              TempString := TempString + Lines[x - 1] + #13#10;
            TempString := TempString + Copy(Lines[BE.Line - 1], 1, BE.Char - 1);
          end;
        smColumn:
          for x := BB.Line to BE.Line do
            TempString := TempString +
              Copy(Lines[x - 1], BB.Char, BE.Char - BB.Char) + #13#10;
        smLine:
          for x := BB.Line to BE.Line do
            TempString := TempString + Lines[x - 1] + #13#10;
      end;
    ...
    SetSelTextPrimitive(Value);
    if TempString <> '' then
      fUndoList.AddChange(crDelete, StartOfBlock, EndOfBlock, TempString,
        fActiveSelectionMode);
...
begin
  ...
  begin
    ...
    begin
      fTextDrawer.SetBaseFont(fGutter.Font);
      fGutter.CharWidth := fTextDrawer.CharWidth - fTextDrawer.CharExtra;
      fTextDrawer.SetBaseFont(Font);
    end
    else
      fGutter.CharWidth := fCharWidth;
    nW := fGutter.RealGutterWidth(fGutter.CharWidth);
...
{$IFOPT J-}{$DEFINE SYN_J_OFF}{$J+}{$ENDIF}
const
  WheelInited: Boolean = False;
{$IFDEF SYN_J_OFF}{$J-}{$UNDEF SYN_J_OFF}{$ENDIF}
var
  PC, P: TPoint;
  nDelta: integer;
...
{$IFDEF SYN_COMPILER_4_UP}
  ...
  if not WheelInited then
    inherited;
  WheelInited := True;
{$ENDIF}

  PC := ScreenToClient(Mouse.CursorPos);
  if (PC.X < 0) or (PC.Y < 0) or (PC.X > Width) or (PC.Y > Height) then
    Exit;
...
begin
  ...
  begin
    ...
    else  // DX < 0. Handle ecLeft/ecPageLeft at BOL.
...
{$ENDIF}
              ...
              begin
                ...
                begin
                  ...
                  if (Pos('remark', attr.Name) <> 0)
                  or (Pos('string', attr.Name) <> 0)
                  or (Pos('comment', attr.Name) <> 0) then
...
begin
  ...
  begin
    for i := 0 to fPlugins.Count - 1 do
      TSynEditPlugin(fPlugins[i]).LinesDeleted(FirstLine, Count, AddUndo);
  end;
end;
...
begin
  if fWordWrapPlugin = nil then
    ...
    Result := Max(fWordWrapPlugin.RowCount, Lines.Count);
  end;
end;
  ...
  begin
    ...
    FoldRange := FoldRangeForLine(ToLine);
		...
		if FoldRegion.AddEnding then
			Lines[FromLine-1] := Lines[FromLine-1] + AnsiStrLower(FoldRegion.Close);
...
begin
	Result := 0;
  if ALine >= Strings.Count then Exit;
...
begin
  ...
  for i := 0 to Count do
      ...
      collapsed within their parents }
        ...
        { We will need this variable to allocate mememory for
  ...
  folds becouse they don't encapsulate the real line numbers }
	...
	{i := 0; // Uncollapsed fold index
	j := 0; // Collapsed fold index
	Count := fAllFoldRanges.AllCount - 1;

	if TemporaryAllFoldRanges.AllCount > 0 then
  begin
		while (i <= Count) do
			if (TemporaryAllFoldRanges[j].ParentCollapsed) or
			(fAllFoldRanges[i].FromLine > TemporaryAllFoldRanges[j].FromLine) then
			begin
				if j = Pred(TemporaryAllFoldRanges.AllCount) then
				begin // If it's a last collapsed fold, add uncollapsed fold at the end of the list
					TemporaryAllFoldRanges.AllRanges.Add(fAllFoldRanges[i]);
					Inc(i);
				end
				else // Otherwise skip to the next collapsed fold to check it
					Inc(j);
			end
			else if fAllFoldRanges[i].FromLine < TemporaryAllFoldRanges[j].FromLine then // This uncollapsed fold should be inserted before current collapsed fold
			begin
  ...
  else // There are no collapsed folds
  ...
  begin
      ...
      begin
    		if fAllFoldRanges[i].FromLine < TemporaryAllFoldRanges[j].FromLine then
      	begin
      		TemporaryAllFoldRanges.AllRanges.Insert(j, fAllFoldRanges[i]);
        	Break;
      	end;
...
var
  ...
  Parents: TList;
  Ptr, TmpPtr, KeyWordPtr, StartPtr: PChar;
          ...
          begin
            ...
            and ((not FoldRegions[i].WholeWords) or (IsWholeWord(Ptr-1, Ptr+1))) then
						begin // Found an open character
							GetHighlighterAttriAtRowCol(CharIndexToRowCol(Ptr - StartPtr), Attr, Attri);
							if Attri <> nil then
								Attr := LowerCase(Attri.Name)
							else
								Attr := '';

							if (Attr <> '') and ((Pos('string', Attr) = 1)
								or (Pos('comment', Attr) = 1) or (Pos('remark', Attr) = 1)) then
								Break;
							...
							begin // Otherwise find the end keyword
								while Ptr^ <> #0 do
									...
									begin
										...
										if (Attr = '') or ((Pos('string', Attr) <> 1)
											and (Pos('comment', Attr) <> 1) and (Pos('remark', Attr) <> 1)) then
											Break
										else
											Inc(Ptr);
									end
...
begin
  ...
  for i := 0 to fAllFoldRanges.AllCount - 1 do
      ...
      and ((FromLine + 1) <= Line) and (ToLine > Line) then
  		begin
  			Result := True;
  			Break;
  		end;
	...
	for i := fAllFoldRanges.AllCount - 1 downto 0 do
  	with fAllFoldRanges[i] do
  		if (RealLevel = ALevel) and (Collapsed) and (not ParentCollapsed) then
    		Uncollapse(Self.fAllFoldRanges[i]);
...
begin
    ...
    begin
    	...
    	for j := 0 to fAllFoldRanges.AllCount - 1 do
          ...
          begin
          	if (Nearest = -1) or (FromLine > Self.fAllFoldRanges[Nearest].FromLine) then 
          		Nearest := j
          end
...
var
  ...
  begin
  	Result := False;
		...
		if (IsKeyWord) and (OpenKeyWord) then // Is there a keyword to highlight at the cursor position
      fHighlightedFoldRange := FoldRangeForLine(CaretY)
    else if (IsKeyWord) and (not OpenKeyWord) then // Is there a keyword to highlight at the cursor position
...
function TCustomSynEdit.IsKeywordAtCursorPos(OpenKeyWord: System.PBoolean): Boolean;
  ...
  var
	...
	begin
		...
		for i := 0 to FoldRegions.Count - 1 do
    begin
    	if IsKeywordAtCursorPosForFoldRegions(OpenKeyWord, FoldRegions[i].SubFoldRegions) then
      begin
      	Result := True;
        Exit;
      end;
...
begin
  ...
  else if CodeFolding.FoldRegions.Count > 0 then
  	Result := IsKeywordAtCursorPosForFoldRegions(OpenKeyWord,
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if fBlockBegin.Char <> 1 then
            Exit;

          if Length(TrimRight(Lines[fBlockEnd.Line - 1])) + 1 > fBlockEnd.Char then
            Exit;

          for i := fAllFoldRanges.AllCount - 1 downto 0 do
            if fAllFoldRanges[i].FromLine = fBlockBegin.Line then
            begin
              fUndoList.AddChange(crDeleteCollapsedFold, StartBC, EndBC,
                '', SelectionMode, fAllFoldRanges[i], i);
    ...
    begin
      ...
      begin
        ...
        else if Length(TrimRight(Lines[CaretY - 1])) + 1 <= CaretX then
        begin
          Result := True;
          if (FoldRange1 <> nil) and (FoldRange1.Collapsed)
          and (not FoldRange1.ParentCollapsed) then
          begin
            Uncollapse(FoldRange1);
            CaretY := FoldRange1.LinesCollapsed + CaretY + 1;
      ...
      begin
        ...
        begin
          ...
          begin
            Uncollapse(FoldRange2);
            CaretY := FoldRange2.LinesCollapsed + CaretY + 1;
          end;
        end
        else if (FoldRange2 <> nil) and (not FoldRange2.Collapsed)
        and (not FoldRange2.ParentCollapsed) then
          Result := True
        else
          Result := False;
      end
      else if (aKey = VK_DELETE)
      and (Length(TrimRight(Lines[CaretY - 1])) + 1 <= CaretX) then
      begin
        Result := True;
      ...
      begin
        ...
        if (FoldRange1 = nil) or (not FoldRange1.Collapsed) then
          Result := True
        else
          Result := False;
      end;
...
begin
	...
	for i := 0 to fAllFoldRanges.AllCount - 1 do
		with fAllFoldRanges[i] do
			if (Self.fAllFoldRanges[i] <> FoldRange) and ((Collapsed)
			and (not ParentCollapsed)) then
...
begin
  { Local change: no DoChange here -- Add only forwards to TList.Add. }
...
begin
  { Local change: Remove maps to TList.Extract and does not call DoChange. }
...
constructor TSynEditCodeFoldingPlugin.Create(AOwner: TCustomSynEdit);
begin
	fOwner := AOwner;
end;

procedure TSynEditCodeFoldingPlugin.LinesDeleted(FirstLine,
  Count: integer; AddUndo: Boolean);
var
  i: Integer;
  StartBC, EndBC: TBufferCoord;
begin
  ...
  begin
    ...
    EndBC.Char := 0;
      ...
      ( (Count > 1) and (fOwner.fAllFoldRanges[i].FromLine >= FirstLine)
      and (fOwner.fAllFoldRanges[i].FromLine <= Pred(FirstLine + Count)) ) then
      begin
        if AddUndo then
...
begin
  for i := fOwner.fAllFoldRanges.AllCount - 1 downto 0 do
  begin
    if FirstLine <= fOwner.fAllFoldRanges[i].fFromLine then
      Inc(fOwner.fAllFoldRanges[i].fFromLine, Count);

    if (not fOwner.fAllFoldRanges[i].fCollapsed)
    and (FirstLine <= fOwner.fAllFoldRanges[i].fToLine) then
      Inc(fOwner.fAllFoldRanges[i].fToLine, Count);
  end;
end;
//### End Code Folding ###
