...
begin
  ...
  if fClose <> nil then
    FreeMem(fClose);
  if fRegExpOpen <> nil then
    FreeMem(fRegExpOpen);
  if fRegExpClose <> nil then
    FreeMem(fRegExpClose);
...
destructor TSkipRegionItem.Destroy;
begin
  if fOpen <> nil then
    FreeMem(fOpen);
  if fClose <> nil then
